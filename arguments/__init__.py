...
class ParamGroup:
    def __init__(self, parser: ArgumentParser, name : str, fill_none = False):
        ...
        for key, value in vars(self).items(): # list the arguments at cmd
            shorthand = False
    ...
    def extract(self, args):
        ...
        for arg in vars(args).items(): # 
            if arg[0] in vars(self) or ("_" + arg[0]) in vars(self):
                setattr(group, arg[0], arg[1]) # set named attr to specific value
        return group
...
class PipelineParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.antialiasing = False # low-pass filter (adds 0.3 variance) to prevent 
